...
let streamedPoints = []
let cityGrids = []  // 从 /api/grids 加载的城市列表
...
function startContourStream() {
  ...
  contourStream.onmessage = (e) => {
    ...
    if (data.type === 'batch') {
      ...
      if (nationalMode === 'contour' && data.contours) {
        renderContourRegions(data.contours, predictions, handleCityClick, pts)
      } else {
        clearContourLayer()
        renderGridHeatmap(pts)
      }
      updateContourSummary(data.summary, data.pointCount)
      const el = document.getElementById('update-time')
      el.style.color = data.degraded ? '#f59e0b' : ''
      el.textContent = `更新于 ${new Date().toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })}${data.degraded ? ' (降级)' : ''}`
      stopContourStream()
      window.__contourReady = true
    } else if (data.type === 'error') {
      const el = document.getElementById('update-time')
      el.style.color = '#ef4444'
      el.textContent = data.message.includes('预热') ? '数据预热中，请稍后...' : `区域加载失败: ${data.message}`
      stopContourStream()
    }
  }
...
function switchToNational() {
  ...
  document.querySelectorAll('#city-buttons .view-btn').forEach(b => b.style.display = b.id === 'btn-national' ? '' : 'none')
...
async function loadCityGrids() {
  try {
    ...
    cityGrids.forEach(g => {
      ...
      btn.addEventListener('click', () => switchToCity(g.id))
      container.appendChild(btn)
    })
    document.getElementById('btn-national')?.classList.add('view-btn-inactive')
  document.title += ' [' + cityGrids.length + 'btns]'
  } catch (e) { console.error("loadCityGrids:", e); document.title += " [BTN ERR]" }
}

function switchToCity(cityId) {
  const cfg = cityGrids.find(g => g.id === cityId)
  if (!cfg || currentView === cityId) return
  document.querySelectorAll('#city-buttons .view-btn').forEach(b => b.classList.add('view-btn-inactive'))
  if (cfg.isNational) {
    document.getElementById('btn-national').classList.remove('view-btn-inactive')
  document.title += ' [' + cityGrids.length + 'btns]'
    switchToNational()
  } else {
    document.getElementById('btn-' + cityId).classList.remove('view-btn-inactive')
    switchToCityGrid(cfg)
  }
}
...
function switchToCityGrid(cfg) {
  ...
  stopContourStream()
  document.getElementById('view-modes').style.display = ''
  document.querySelectorAll('#city-buttons .view-btn').forEach(b => b.style.display = '')
  document.querySelectorAll('#view-modes .view-mode-btn').forEach(btn => {
    btn.style.display = btn.dataset.mode === 'contour' ? 'none' : ''
  })
  closePanel()
  const map = getMap()
  if (map) { clearGridLayer(); map.setView(cfg.center, cfg.zoom) }
  showLoading('正在加载' + cfg.name + '区域网格数据...')
  loadGridForCity(cfg.id)
}

async function loadGridForCity(cityId) {
  try {
    const res = await fetch('/api/grid/' + cityId)
    if (!res.ok) throw new Error('HTTP ' + res.status)
    const data = await res.json()
    if (!data.points || !data.points.length) { showError("暂无网格数据"); return }
...
document.addEventListener("DOMContentLoaded", async () => { try {
  initMap('map'); try { await loadCityGrids() } catch(e) { console.error("城市列表加载失败:", e) }

  document.getElementById('refresh-btn').addEventListener('click', () => {
    stopContourStream()
    if (currentView !== "national" && cityGrids.length) {
      showLoading('正在刷新杭州区域数据...')
      loadGridForCity(currentView)
    } else {
      contourData = null
      loadNationalData()
    }
  ...
  document.getElementById('view-modes').addEventListener('click', async (e) => {
    ...
    if (mode === 'markers' && predictions) {
      ...
      clearContourLayer()
      if (contourData && contourData.points) { renderGridHeatmap(contourData.points) }
      else { startContourStream() }
  ...
  setInterval(() => {
    if (currentView !== "national" && cityGrids.length) loadGridForCity(currentView)
    else if (nationalMode !== 'contour') loadNationalData()
  }, 5 * 60 * 1000)
  } catch (e) { document.title = "ERR: " + e.message; console.error(e); }
})
